...
//  MadgwickQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f,  my,  mx, mz);
  ...
  if (!AHRS)
  {
    ...
    if (myIMU.delt_t > 1000/frequency) // UPDATE ONLY FOR TESTING
    {
      if(SerialDebug)
      {
...
//      digitalWrite(myLed, !digitalRead(myLed));  // toggle led
  ...
  else
  {
    ...
    if (myIMU.delt_t > 1000/frequency) // UPDATE THIS FOR SERIAL FREQUENCY
    {
...
// which has additional links.
      ...
      if (bluetooth) {
        String string1 = "AT+BLEUARTTXF=A";
        string1 += analogRead(thumb);
        string1 += " ";
        string1 += analogRead(idx);
        string1 += " ";
        string1 += analogRead(middle);
        string1 += " ";
        string1 += analogRead(ring);
        string1 += " ";
        string1 += analogRead(pinky);
        String string2 = "AT+BLEUARTTXF=B";
        string2 += myIMU.roll;
        string2 += " ";
        string2 += myIMU.pitch;
        string2 += " ";
        string2 += myIMU.yaw;
        String string3 = "AT+BLEUARTTXF=C";
        string3 += myIMU.gx;
        string3 += " ";
        string3 += myIMU.gy;
        string3 += " ";
        ...
        if (ble.atcommand(F("AT+BLEGETRSSI")) != 0) {
          string1.toCharArray(message1, sizeof(message1));
          ble.atcommand(F(message1));
          string2.toCharArray(message2, sizeof(message2));
          ble.atcommand(F(message2));
          string3.toCharArray(message3, sizeof(message3));
          ble.atcommand(F(message3));
        }
      }
      else {
        if (flexSensors)  // send out data for flex sensors if attached
        {
          ...
          Serial.print(analogRead(idx)); Serial.print(" "); // Index finger
