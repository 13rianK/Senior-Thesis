...
xlabel('k');
ylabel('% Correctly Recognized')
title('kNN Results for Varying Values of k (distance calculated without square root)')
...
xlabel('k');
ylabel('% Correctly Recognized')
title('kNN Results for Varying Values of k (distance calculated with square root)')
...
xlabel('k');
ylabel('% Correctly Recognized')
title('kNN Results on for Varying Values of k (Discrete binary values for fingers angle)')
...
index(1) = mean(a180(:,2));
err(1) = std(a180(:,2));
e1 = errorbar(X, index*0.0049, err*0.0049, 'linewidth', 2);
e1.Color = 'red';
...
err(1) = std(a180(:,3));
e2 = errorbar(X, middle*0.0049, err*0.0049, 'linewidth', 2);
e2.Color = 'blue';
...
ring(1) = mean(a180(:,4));
err(1) = std(a180(:,4));
e2 = errorbar(X, ring*0.0049, err*0.0049, 'linewidth', 2);
e2.Color = 'green';
...
pinky(1) = mean(a180(:,5));
err(1) = std(a180(:,5));
e2 = errorbar(X, pinky*0.0049, err*0.0049, 'linewidth', 2);
e2.Color = 'magenta';

legend('Index','Middle','Ring','Pinky');
xlabel('Angle of Hand (?)');
ylabel('Voltage Measurement (V)');
axis([80 190 1.5 4]);
title('Flex Sensor Angle Characterization');
